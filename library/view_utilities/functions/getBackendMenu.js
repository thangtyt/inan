...
function addMenuItem(menu,permissions,group,currentUrl,app){
    ...
    _.map(menu.sorting, function (key) {
        ...
        if ( ['blog', 'configuration', 'menu','widgets','plugins'].indexOf(key) == -1 && _.has(menu[group]['features'], key) && _.has(menu[group]['features'][key], 'menus') && !_.isUndefined(permissions["feature"][key])) {
